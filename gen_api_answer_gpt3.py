...
Usage:
python gen_api_answer_gpt3.py --model gpt3
"""
...
def chat_completion_mamba(model, inp, temperature, max_tokens):
    ...
    return (response_data.get('text', '')[0][len(inp):])[:(response_data.get('text', '')[0][len(inp):]).index("Query")]

def format_mamba_input(question, prev_answer=None):
    """
    Format the input prompt for the Mamba API, combining the URIAL template and the conversation history.
    
    Args:
        question (dict): A dictionary containing the conversation turns.
        prev_answer (str): The previous answer from the model, if any.
    
    Returns:
        str: The formatted input prompt for the model.
    """
    # For the first turn, just include the query from the first question
    if prev_answer is None:
        prompt = URIAL_TEMPLATE.format(query=question['turns'][0])  # First question only
    else:
        # For subsequent turns, include the previous answer and the next question
        prompt = URIAL_TEMPLATE.format(query=question['turns'][0])
        prompt += TURN_TEMPLATE.format(answer=prev_answer, query=question['turns'][1])
        
    print(prompt)  # Debug: print the final prompt being sent
    return prompt
...
def get_answer(question, model, num_choices, max_tokens, answer_file):
    ...
    for i in range(num_choices):
        turns = []  # Reset turns for each choice

        for turn_idx in range(2):  # Loop for 2 turns in the conversation
            if turn_idx == 0:
                # First turn, format only with the first question
                inp = format_mamba_input(question)
            else:
                # Subsequent turn, format with the previous answer and the next question
                inp = format_mamba_input(question, turns[-1])  # Use the previous answer
            
            print(f"Input for API Call: {inp}")  # Debug: print the input being sent

            # Call the model API to get a response
            output = chat_completion_mamba(model, inp, temperature, max_tokens)
            print(f"Output from API Call: {output}")  # Debug: print the output received

            # Strip any extra whitespace and append the new answer to the turns list
            answer = output.strip()
            turns.append(answer)  # Add each turn (answer) to the conversation

        # After looping through both turns, store the choice with the sequence of turns
        choices.append({
            "index": i,        # Choice index
            "turns": turns.copy()  # Use a copy to avoid mutability issues across choices
        })

    # Save the final answer structure
    ans = {
        "question_id": question["question_id"],
        "answer_id": shortuuid.uuid(),
        "model_id": model,
        "choices": choices,  # Contains each turn stored separately for each choice
        "tstamp": time.time(),
    }

    # Ensure the output directory exists and save the answer to a file
